...
let rec remove_stutter l = match l with 
  ...
  | a :: [] -> a :: []
  | a :: b :: tail ->
      if a = b then remove_stutter (a :: tail)
...
let rec removeSetsDuplicates a b = 
  match a with 
  | [] -> []
  | h::t -> if (elem h b) then (removeSetsDuplicates t b) else (h :: (removeSetsDuplicates t b))

let rec union a b =
  (removeSetsDuplicates a b) @ b

let rec diff a b =
  removeSetsDuplicates a b
...
let rec apHelp n c = 
  if n < 10 then c
  else apHelp (sum (digitsOfInt n)) (c+1)
