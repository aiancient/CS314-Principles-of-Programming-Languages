...
def assoc_list (l):
    count = [reduce(lambda x, y: x + (1 if y == a else 0), l, 0) for a in l]
    return list(set(zip(l,count)))
...
def buckets (f, l):
    ...
    for sublist in groups:
        while groups.count(sublist) > 1:
            ri = groups[::-1].index(sublist) 
            del groups[len(groups) - 1 - ri] 
...
def pathSum(root: TreeNode, targetSum: int) -> List[List[int]]:
    def total(root,ans,answer,targetSum):
        if root is None:
            return
        ...
        total(root.left,ans+[root.val],answer,targetSum-root.val)
        total(root.right,ans+[root.val],answer,targetSum-root.val)
        return answer
    ...
    ans=[]
    answer=[]
    return total(root,ans,answer,targetSum)
